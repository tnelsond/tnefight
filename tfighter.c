...
void tfighter_update(tfighter *t, tlevel *tl){
	...
	for(i=0; i<tl->MAX_BOXES; ++i){
		...
		if((owner != NULL) && owner != t && box->tick > box->maxdelay && !(box->hit & t->id) && intersects(&t->rect, &box->rect)){
			...
			t->state |= HITSTUN;
			t->tick = (int)(box->kbgrowth * t->damage / 4);
			t->damage += box->attack;
